...
- **Sprint #5**: 🎯 **Mejoras en Formularios y registro de máquina** - ReactHookForms + Wizard Component + Alta máquinas
   - **🎯 FOCO PRINCIPAL**: Capitalizar sobre navegación 12.4a ya implementada en Sprint #4 estableciendo infraestructura de formularios escalable. Implementar ReactHookForms + Wizard Component reutilizable + registro de máquina como caso de uso. Frontend: sistema formularios robusto + wizard multi-step + validación tiempo real + UX patterns. Backend: modelo máquina + endpoints + middleware autorización. **CONSTRUYE HACIA**: Base formularios sólida para CRUD completo Sprint #6 y todos los formularios futuros.
...
- **Sprint #8**: 🎯 **Machine Enhancement + PWA Base + QuickCheck Refinement** - Mejoras críticas post-MVP QuickCheck y fundación PWA
   - **🎯 FOCO PRINCIPAL**: Capitalizar sobre QuickCheck Sprint #7 con refinamientos de valor inmediato + establecer base PWA. Machine enhancement: campo assignedTo + usageRate (dailyHours/weeklyDays) + fixes (powerSource backend, display tipo, machinePhotoUrl básico). QuickCheck tracking: metadata responsable (technicianName/Id) para auditoría. PWA: manifest + icons + service worker básico + testing multi-dispositivo. Navigation drawer: navegación global responsive. Azure fix crítico: routing 404 en refresh. **CONSTRUYE HACIA**: Base sólida para alertas basadas en uso real Sprint #9 + PWA funcional + tracking compliance.

- **Sprint #9**: � **Centro de Notificaciones Completo** - Sistema completo front/back de bandeja de notificaciones multi-source con primera integración funcional
   - **🔔 FOCO PRINCIPAL**: Establecer sistema de notificaciones pragmático y extensible construyendo sobre infraestructura Sprint #8. Arquitectura por capas: Domain/Persistence (Notification como subdocumento User), Application Layer (4 Use Cases modulares: Add, Get, MarkAsSeen, Count), Frontend UI (badge navbar, lista, toasts react-hot-toast), Observer Pattern (TanStack Query cache listener detecta notificaciones nuevas → dispara toasts automáticos). Primera integración real: QuickCheck fail → AddNotificationUseCase → notificación en BD → Observer detecta → toast + badge actualizados. Polling cada 30s con TanStack Query refetchInterval. Patrón documentado facilita integración futura con Alertas (Sprint #10) y Eventos (Sprint #11). **CONSTRUYE HACIA**: Base crítica y replicable para todos los módulos que emitan notificaciones.
