...
export const MachineSelectModal: React.FC<MachineSelectModalProps> = ({
  ...
  console.log('Machines in Modal:', machines);
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'success' | 'warning' }> = {
      ...
      INACTIVE: { label: t('machines.statuses.inactive'), variant: 'secondary' },
      MAINTENANCE: { label: t('machines.statuses.maintenance'), variant: 'warning' },
      OUT_OF_SERVICE: { label: t('machines.statuses.broken'), variant: 'destructive' },
    };
  ...
  return (
    <Modal
      ...
      description={actionType ? t('quickActions.selectMachineDesc', { action: actionType }) : undefined}
    >
      <div className="flex flex-col gap-4">
		...
		{!isLoading && filteredMachines.length > 0 ? <div>true</div> : <div>false</div>}
		<div>{filteredMachines.length}</div>
        {/* Lista de máquinas */}
            ...
            {filteredMachines.map((machine) => (
              ...
              >
                <CardContent className="p-4">
                  {/* Foto de la máquina */}
                  <div className="mb-3 h-32 w-full bg-muted rounded overflow-hidden">
                  ...
                  <div className="space-y-2">
                    ...
                    <BodyText size="small" className="text-muted-foreground line-clamp-1">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {machine.nickname && (
                        <BodyText size="small" className="text-muted-foreground truncate">
                          {machine.brand}
                        </BodyText>
                      )}
                    </div>
