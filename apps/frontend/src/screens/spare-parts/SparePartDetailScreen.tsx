...
export function SparePartDetailScreen() {
  const { id: machineId, sparePartId } = useParams<{ id: string; sparePartId: string }>();
  ...
  // ========================
    ...
    const createdDate = new Date(part.createdAt).toLocaleDateString(undefined, {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
    
    const updatedDate = new Date(part.updatedAt).toLocaleDateString(undefined, {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
    ...
    content = (
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link to={`/machines/${machineId}/spare-parts`} className="hover:text-foreground">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4">
          <div>
            <Heading1 size="headline" className="tracking-tight text-foreground mb-2">
              {part.name}
            </Heading1>
            <BodyText className="text-muted-foreground">
          ...
          <div className="flex gap-3">
            <Button variant="outline" onPress={vm.actions.handleEdit}>
              <Edit className="h-4 w-4 mr-2" />
              {vm.t('common.edit')}
            </Button>
            <Button variant="outline" onPress={vm.actions.handleOpenOptions}>
