...
export const ExamplesScreen: React.FC = () => {
  const handleShowLoading = () => modal.showLoading('Procesando información...');
  ...
  const handleShowCustom = () =>
    modal.show({
      ...
      description: 'Este modal usa la configuración tradicional con botones propios.',
      content: (
        <div className="text-sm text-muted-foreground">
          Aquí podrías renderizar formularios, listas u otro contenido específico.
        </div>
      ),
      showConfirm: true,
      showCancel: true,
  ...
  return (
    <div className="space-y-8 px-6 py-8 max-w-5xl mx-auto">
      <header className="space-y-2">
        <h1 className="text-3xl font-bold text-foreground">Pantalla de Ejemplos</h1>
        <p className="text-muted-foreground">
          Usa estos controles para probar los distintos modales y componentes UI.
        </p>
      </header>

      <section className="bg-card rounded-lg shadow p-6 space-y-4">
        <h2 className="text-lg font-semibold text-foreground">Modales de feedback</h2>
      ...
      <section className="bg-card rounded-lg shadow p-6 space-y-4">
        <h2 className="text-lg font-semibold text-foreground">TimerButton & Skeleton demos</h2>
        <TimerButtonExamples />
