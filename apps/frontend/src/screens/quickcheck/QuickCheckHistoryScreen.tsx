    ...
    <div className="container mx-auto px-4 py-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
          <Link to={vm.paths.machines} className="hover:text-foreground">Máquinas</Link>
          <span>/</span>
          <Link to={vm.paths.machineDetail} className="hover:text-foreground">Detalle</Link>
          <span>/</span>
          <Link to={vm.paths.quickCheck} className="hover:text-foreground">QuickCheck</Link>
          <span>/</span>
      ...
      {vm.error && (
        <Card className="p-6">
          <div className="text-center py-8">
            ...
            <p className="mt-2 text-muted-foreground">{vm.error instanceof Error ? vm.error.message : 'Ocurrió un error inesperado'}</p>
          </div>
        </Card>
      )}
            ...
            return (
              <Card key={index} className="p-5 hover:shadow-lg transition-shadow">
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${vm.getResultBadgeClasses(record.result)}`}>
...
const QuickCheckDetailModal: React.FC<{ 
  ...
  const renderResultIcon = (type: 'approved' | 'disapproved' | 'omitted' | 'unknown') => {
    switch (type) {
      case 'approved':
        return (
